<!DOCTYPE html>
<html lang="en"  ng-app="vacationFormApp">
   <head>
      <meta charset="utf-8">
...
<i class="fa fa-refresh fa-spin fa-3x"
      ...
      <section class="bg-primary" id="about">
         <div class="container">
            <div class="row">
             <div class="title-createplan" >Vacation<button class="btn btn-primary pull-right" ng-click="goSelectGoal()">Back</button></div>
               <div >
...
<a  class="active-menu"><i class="fa fa-arrow "></i>Details</a>
					...
					<form name="goalDetails" id="goalForm" >
                       ...
                       <p>How much would you like to budget for your vacation?
  									<input type="text" class="form-control" name="amountSave" maxlength="15" ng-model="formdata.amountSave" onkeypress="return numbersonly(event)"/>
								</div></p>
								<div class="form-group" align="left" ng-show="show1>2">
       							<p>When do you want to take this vacation?</p>
       				
                    			    	<select class="form-control" ng-model="formdata.vacationYear"
											style="width: 90px; margin-right: auto"><option
												ng-repeat="vacationYear in vacationYear1" value="{{vacationYear.number}}">{{vacationYear.number}}</option></select>
                    			   </div> 	
								
								<div class="form-group" align="left" ng-show="show1>3"><p> Is this going to be one time, or recurring?
  									<select  name="frequency" ng-model="formdata.frequency" class="form-control"   >
                        		<option ng-selected="frequency.number == formdata.frequency " ng-repeat="frequency in frequency" ng-value="frequency.number" >{{frequency.string}}</option>			
                    			  </select></p> 
									<div class="slider"></div>
								</div>	
								<!-- <div class="form-group" align="left" ng-show="show1>3"> <p> Monthly Debit 
  									<input type="text" class="form-control" name="debitInfo" value="$0" ng-model="formdata.debitInfo"></p>
								...
								<div class="form-group" align="left" ng-show="show1>2">
 								 ...
 								 <div id="demo" class="collapse">
 									...
 									&nbsp&nbsp<button type="button"  ng-click="processForm1(); calculateTotal()"class="btn btn-primary">Update</button> 
								</div>
  ...
  <div class="progress-bar progress-bar-striped active" id="progress_bar" ng-model="progressbar"  aria-valuemin="0" aria-valuemax="100" style="width:2%">
                		...
                		<span style="color:red; font-size: 90%;">{{msgerr}}</span>	
       							<p>How much would you like to budget for your vacation?</p> 
       							<div class="input-prepend">
 									<span class="money-fieldmodel">
       							<input type="text" placeholder="20000" name="amountSave"  maxlength="15" ng-model="formdata.amountSave" onkeypress="return numbersonly(event)"/></br></br>
       							</span>
       							</div>
       							<button type="button" class="btn btn-primary" onclick="progress_bar()"  ng-click="progressBar();processForm()" >Next</button></br></br>
    						</div>
   							<span></span>
       							...
       							<p>When do you want to take this vacation?</p>
                    			    ...
                    			    <select class="form-control" ng-model="formdata.vacationYear"
														style="width: 90px; margin-left: auto; margin-right: auto"><option
															ng-repeat="vacationYear in vacationYear1" value="{{vacationYear.number}}">{{vacationYear.number}}</option></select></br>
                    			    <br>
                    			    <button type="button" class="btn btn-primary" onclick="progress_bar()" ng-click="progressBar2()">Next</button></br></br>
   							 
       							</div> 
    							 <span></span>
        					 <div class="form-group" ng-show="show==4" >
        					 <span style="color:red; font-size: 90%;">{{msgerr}}</span>
      							 <p>Is this going to be one time, or recurring?</p>
       							<select  name="frequency" ng-model="formdata.frequency" class="form-control" style="width:170px;    margin-left: auto;    margin-right: auto;" >
                        		<option ng-selected="frequency.string == formdata.frequency " ng-repeat="frequency in frequency" ng-value="frequency.number" >{{frequency.string}}</option>			
                    			    ...
                    			    <br>
                    			    <button type="button" class="btn btn-primary" onclick="progress_bar()" ng-click="progressBar1()">Next</button></br></br>
   							 </div>
    				...
    				<!-- /.modal -->
    						<div class="right_content">
								...
								<p>You have to save $ {{ messresult }} annually to attain the Vacation goal.</p>
								<table class="table" >
								<thead>
								</thead>
								<tbody>
