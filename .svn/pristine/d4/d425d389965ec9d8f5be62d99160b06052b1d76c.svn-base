angular.module('wealthsetter').controller('CarloanCtrl', ['$scope','$rootScope','$uibModal', '$log','$http','$state' ,CarloanCtrl]);
function CarloanCtrl($scope,$rootScope,$uibModal, $log,$http,$state,$uibModalInstance) {  
	                $.material.init();
					$scope.show = 1;
				...
				$http({
					...
					$scope.calculate = function() {
						if ( $scope.formdata.carPrice == null
							...
							$scope.clc_msgerr = "Interest rate should be less than or equal to hundred percentage";
						}
							...
							$scope.clc_msgerr = "Sale tax should be less than or equal to hundred percentage";
						}
							...
							$http( {
									...
									.then( function( result ) {
												...
												console.log( result.data );
												if ( result.data.status == "success" ) {
												/*	$state.go('calculator.carloanresult');*/
													//alert(result.data);
